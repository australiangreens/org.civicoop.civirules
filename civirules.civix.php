 ...
 * extension.
 */
class CRM_Civirules_ExtensionUtil {
  const SHORT_NAME = 'civirules';
  const LONG_NAME = 'org.civicoop.civirules';
  const CLASS_PREFIX = 'CRM_Civirules';
 ...
 */
function _civirules_civix_civicrm_install() {
  _civirules_civix_civicrm_config();
  if ($upgrader = _civirules_civix_upgrader()) {
    $upgrader->onInstall();
  }
}
 ...
 */
function _civirules_civix_civicrm_postInstall() {
  _civirules_civix_civicrm_config();
  if ($upgrader = _civirules_civix_upgrader()) {
 ...
 */
function _civirules_civix_civicrm_uninstall() {
  _civirules_civix_civicrm_config();
  if ($upgrader = _civirules_civix_upgrader()) {
 ...
 */
function _civirules_civix_civicrm_enable() {
  _civirules_civix_civicrm_config();
  if ($upgrader = _civirules_civix_upgrader()) {
 ...
 * @return mixed
 */
function _civirules_civix_civicrm_disable() {
  _civirules_civix_civicrm_config();
  if ($upgrader = _civirules_civix_upgrader()) {
 ...
 */
function _civirules_civix_civicrm_upgrade($op, CRM_Queue_Queue $queue = NULL) {
  if ($upgrader = _civirules_civix_upgrader()) {
  ...
  if (empty($path)) {
    ...
    foreach ($menu as $key => &$entry) {
      if ($entry['attributes']['name'] == $first) {
        ...
        $found = _civirules_civix_insert_navigation_menu($entry['child'], implode('/', $path), $item);
      }
    }
    return $found;
  }
